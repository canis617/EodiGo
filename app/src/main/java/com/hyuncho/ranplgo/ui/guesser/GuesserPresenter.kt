...
class GuesserPresenter : GuesserContract.Presenter {

    private var guesserView : GuesserContract.View? = null

    lateinit var fusedLocationProviderClient: FusedLocationProviderClient
    lateinit var locationRequest: LocationRequest
    lateinit var locationCallback: GuesserActivity.MyLocationCallBack
    ...
    override fun attachView(view: GuesserContract.View) {
        guesserView = view
    }
    ...
    override fun gameCreate(distance: String, currentLocation : Location) {
        ...
        guesserView?.updateMap(startLocation, circleMarker)
    }
    ...
    override fun guess(context:Context, range : Int){
        ...
        if (selectedLocation != null){
            ...
            nextIntent.putExtra("selectedLat", selectedLocation?.latitude)
            nextIntent.putExtra("selectedLng", selectedLocation?.longitude)
            nextIntent.putExtra("range", range)
            nextIntent.putExtra("mode", "guesser")
            context.startActivity(nextIntent)
        }
    }

    override fun updateSelected(selectedLocation: LatLng) {
...
// 외부 저장소 읽기가 허용되지 않았다면
    ...
    override fun locationInit(context : Context) {
        ...
        // 현재 사용자 위치를 저장.
        locationCallback = (context as GuesserActivity).MyLocationCallBack() // 내부 클래스 조작용 객체 생성
